...
// Get proje durumu terms for status filter
$durum_terms = get_terms(array(
    'taxonomy' => 'proje_durumu',
    'hide_empty' => false,
    'orderby' => 'name',
    'order' => 'ASC',
));
if (is_wp_error($durum_terms)) $durum_terms = array();
...
<main class="w-full bg-background-dark">
    ...
    <!-- Status Filters (Tamamlanan / Devam Eden) -->
    <?php if (!empty($durum_terms)): ?>
        <section class="z-40 bg-background-dark border-b border-white/5 py-6">
            <div class="mx-auto max-w-[1440px] px-6 lg:px-12">
                <div class="flex gap-3 overflow-x-auto no-scrollbar items-center md:justify-center pb-2 md:pb-0">
                    <button
                        class="status-filter-btn active shrink-0 h-10 px-6 rounded-full bg-primary text-navy-dark text-xs font-bold uppercase tracking-widest transition-all hover:scale-105"
                        data-status="all">
                        Tüm Projeler
                    </button>
                    <?php foreach ($durum_terms as $durum): ?>
                        <button
                            class="status-filter-btn shrink-0 h-10 px-6 rounded-full bg-white/5 border border-white/10 text-white/80 text-sm font-medium hover:bg-white/10 hover:text-white hover:border-white/20 transition-all"
                            data-status="<?php echo esc_attr($durum->slug); ?>">
                            <?php echo esc_html($durum->name); ?>
                        </button>
                    <?php endforeach; ?>
                </div>
            </div>
    ...
    <?php if (!empty($child_cats)): ?>
        <section class="top-[80px] z-40 bg-background-dark/95 backdrop-blur-sm border-b border-white/5 py-4">
            <div class="mx-auto max-w-[1440px] px-6 lg:px-12">
                <div class="flex gap-3 overflow-x-auto no-scrollbar items-center md:justify-center pb-2 md:pb-0">
                    <button
                        class="project-filter-btn active shrink-0 h-9 px-5 rounded-full bg-white/10 border border-white/20 text-white text-xs font-medium uppercase tracking-widest transition-all"
                        data-filter="all">
                        Tümü
                    </button>
                    <?php foreach ($child_cats as $cat): ?>
                        <button
                            class="project-filter-btn shrink-0 h-9 px-5 rounded-full bg-white/5 border border-white/10 text-white/60 text-xs font-medium hover:bg-white/10 hover:text-white hover:border-white/20 transition-all"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-20 lg:gap-y-32" id="projects-grid">
            <?php if ($projects_query->have_posts()): ?>
                ...
                while ($projects_query->have_posts()):
                    ...
                    // Get proje durumu
                    $durum_list = wp_get_post_terms(get_the_ID(), 'proje_durumu', array('fields' => 'all'));
                    $durum_slugs = array();
                    $durum_names = array();
                    if (!is_wp_error($durum_list)) {
                        foreach ($durum_list as $d) {
                            $durum_slugs[] = $d->slug;
                            $durum_names[] = $d->name;
                        }
                    }
                    $durum_attr = implode(' ', $durum_slugs);
                    ...
                    <div class="project-item group flex flex-col gap-6 cursor-pointer <?php echo $is_offset ? 'md:mt-24' : ''; ?>"
                        ...
                        data-status="<?php echo esc_attr($durum_attr); ?>">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-wrap items-center gap-4 text-sm text-gray-400 font-medium tracking-wide">
                                ...
                                if (!empty($display_cats)):
                                    ...
                                    <span class="flex items-center gap-1">
                                        <span class="w-1 h-1 rounded-full bg-white/20"></span>
                                        <span class="text-white/80"><?php echo esc_html(implode(', ', $display_cats)); ?></span>
                                    </span>
                                <?php endif; ?>
                            </div>
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function applyFilters() {
            projectItems.forEach(function(item) {
                var matchStatus = (activeStatus === 'all') || (item.getAttribute('data-status') || '').indexOf(activeStatus) !== -1;
                var matchCategory = (activeCategory === 'all') || (item.getAttribute('data-categories') || '').indexOf(activeCategory) !== -1;
        ...
        statusBtns.forEach(function(btn) {
            btn.addEventListener('click', function() {
                statusBtns.forEach(function(b) {
                    b.classList.remove('bg-primary', 'text-navy-dark', 'active');
                ...
                this.classList.add('bg-primary', 'text-navy-dark', 'active');
        ...
        filterBtns.forEach(function(btn) {
            btn.addEventListener('click', function() {
                filterBtns.forEach(function(b) {
                    b.classList.remove('bg-white/10', 'border-white/20', 'text-white', 'active');
                    b.classList.add('bg-white/5', 'border-white/10', 'text-white/60');
                });
                this.classList.remove('bg-white/5', 'border-white/10', 'text-white/60');
                this.classList.add('bg-white/10', 'border-white/20', 'text-white', 'active');
